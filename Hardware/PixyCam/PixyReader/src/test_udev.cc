...
int main (void)
{
	...
	udev_enumerate_add_match_sysattr(enumerate, "idVendor", "b1ac");
	...
	udev_list_entry_foreach(dev_list_entry, devices) {
		...
		printf("Device Dev Path: %s\n", udev_device_get_devpath(dev));
		printf("Device Sys Path: %s\n", udev_device_get_syspath(dev));
    ...
    while (properties_list) {
      printf("  property %s = %s\n", udev_list_entry_get_name(properties_list), udev_list_entry_get_value(properties_list));
    ...
    while (tags_list) {
      printf("  tag name = %s\n", udev_list_entry_get_name(tags_list));
