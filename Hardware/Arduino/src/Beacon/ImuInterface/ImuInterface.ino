...
static const uint8_t send_every_nth_message = 2;
static const uint8_t imu_packet_template_size = 20;
static const uint8_t imu_packet_size = imu_packet_template_size * send_every_nth_message;

// packet structure for tranmitting IMU readings
const uint8_t imuPacketTemplate[imu_packet_template_size] = {
  '$',                    // Starting code
  0x00,                   // Message number
...
void setup() {
    ...
    // Copy the template into imuPacket
    for (uint8_t i=0; i<send_every_nth_message; i++) {
      for (uint8_t j=0; j<imu_packet_template_size; j++) {
        imuPacket[i*imu_packet_template_size + j] = imuPacketTemplate[j];
      }
    }
...
void loop() {
    ...
    if (!dmpReady) {
      ...
      // otherwise, check for DMP data ready interrupt (this should happen frequently)
          ...
          packet_counter++;   // loops at 0xFF on purpose
          send_counter = (send_counter+1) % send_every_nth_message;
          uint8_t packet_begin_marker = send_counter * imu_packet_template_size;

          // Copy packet counter to imuPacket
          imuPacket[packet_begin_marker + 16] = packet_counter;
          
          // Copy quaternion data to imuPacket
          imuPacket[packet_begin_marker + 2] = fifoBuffer[0];
          imuPacket[packet_begin_marker + 3] = fifoBuffer[1];
          imuPacket[packet_begin_marker + 4] = fifoBuffer[4];
          imuPacket[packet_begin_marker + 5] = fifoBuffer[5];
          imuPacket[packet_begin_marker + 6] = fifoBuffer[8];
          imuPacket[packet_begin_marker + 7] = fifoBuffer[9];
          imuPacket[packet_begin_marker + 8] = fifoBuffer[12];
          imuPacket[packet_begin_marker + 9] = fifoBuffer[13];

          // Copy acceleration data to imuPacket
          imuPacket[packet_begin_marker + 10] = fifoBuffer[28];
          imuPacket[packet_begin_marker + 11] = fifoBuffer[29];
          imuPacket[packet_begin_marker + 12] = fifoBuffer[32];
          imuPacket[packet_begin_marker + 13] = fifoBuffer[33];
          imuPacket[packet_begin_marker + 14] = fifoBuffer[36];
          imuPacket[packet_begin_marker + 15] = fifoBuffer[37];
          
          if (send_counter == 0) {
            series_counter++;
            imuPacket[packet_begin_marker + 1] = series_counter;
            //Serial1.write(imuPacket, imu_packet_size);
            Serial.write(imuPacket, imu_packet_template_size);
          }

          /*// send all captured data to the serial port as text
