...
void setup(){
  ...
  */
  ...
  //default_address_change_example2();  
...
void loop()
{
  ...
  error = start_sensor(222);    //Start the sensor and collect any error codes.
  error2 = start_sensor(224);    //Start the sensor and collect any error codes.
  
  delay(100);
  
  if (!error){                  //If you had an error starting the sensor there is little point in reading it as you will get old data.
    range = read_sensor(222);   //reading the sensor will return an integer value -- if this value is 0 there was an error
  }
  
  //Take a range reading at the default address of 224
  if (!error2){                  //If you had an error starting the sensor there is little point in reading it as you will get old data.
    range2 = read_sensor(224);   //reading the sensor will return an integer value -- if this value is 0 there was an error
  }
...
// Code Example: Change the default address //
//////////////////////////////////////////////
void default_address_change_example2(){
  boolean error = 0;  //Create a bit to check for catch errors as needed.
  int range;
