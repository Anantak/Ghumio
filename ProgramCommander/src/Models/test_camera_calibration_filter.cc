...
int main(int argc, char** argv) {
  ...
  anantak::CameraIntrinsicsFilter::Options
      camera_intrinsics_filter_options(model_config_.config_file());
  ...
  // Create the filter
  anantak::CameraIntrinsicsFilter
      camera_intrinsics_filter(camera_intrinsics_filter_options, file_msgs_keeper.DataStartTime());
  
  // Iteration interval from the filter settings
  int64_t iteration_interval = camera_intrinsics_filter.IterationInterval(); // microsec
  ...
  while (file_msgs_keeper.MoreDataLeft()) {
    ...
    //  Save data to disk = !realtime. In realtime, we skip saving to disk
    camera_intrinsics_filter.RunIteration(iteration_end_ts, new_msgs, !run_in_realtime);
    
    // Check if should continue
    if (camera_intrinsics_filter.Finished()) {
