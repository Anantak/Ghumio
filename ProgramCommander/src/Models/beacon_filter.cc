...
static std::string beacon_calibrator_none_hist_command_ = "";
static std::string beacon_calibrator_none_live_command_ = "";
static std::string beacon_calibrator_camera00_hist_command_ = "";
static std::string beacon_calibrator_camera00_live_command_ = "";
static std::string beacon_calibrator_camera01_hist_command_ = "";
static std::string beacon_calibrator_camera01_live_command_ = "";
...
bool ProcessModelCommand(const std::string& cmd_str, std::string& model_name, bool& is_live) {
  if (cmd_str == beacon_calibrator_none_hist_command_ ||
      cmd_str == beacon_calibrator_none_live_command_) {
    model_name = "";
    is_live = true;
    return true;
  }
  else if (cmd_str == beacon_calibrator_camera00_live_command_) {
    model_name = "Camera00.Intrinsics";
    is_live = true;
    return true;
  }
  else if (cmd_str == beacon_calibrator_camera00_hist_command_) {
    model_name = "Camera00.Intrinsics";
    is_live = false;
    return true;
  }
  else if (cmd_str == beacon_calibrator_camera01_live_command_) {
    model_name = "Camera01.Intrinsics";
    is_live = true;
    return true;
  }
  else if (cmd_str == beacon_calibrator_camera01_hist_command_) {
    model_name = "Camera01.Intrinsics";
    is_live = false;
    return true;
  }
...
bool CreateModel(const std::string& model_name, const int64_t& start_ts) {
  ...
  if (model_name == "Camera00.Intrinsics" || model_name == "Camera01.Intrinsics") {
    ...
    anantak::BeaconFilter::Options beacon_filter_options(model_config.config_file());
    ...
    if (calib_msg) {
      ...
      beacon_filter_options.SetStartingCameraIntrinsics(*calib_msg);
    }
    std::unique_ptr<anantak::Model> model_ptr(new anantak::BeaconFilter(beacon_filter_options, start_ts));
...
int main(int argc, char** argv) {
  ...
  // Set command strings
  beacon_calibrator_none_hist_command_ = "COMMAND "+component_name+" model None Hist";
  beacon_calibrator_none_live_command_ = "COMMAND "+component_name+" model None Live";
  beacon_calibrator_camera00_hist_command_ = "COMMAND "+component_name+" model Camera00 Hist";
  beacon_calibrator_camera00_live_command_ = "COMMAND "+component_name+" model Camera00 Live";
  beacon_calibrator_camera01_hist_command_ = "COMMAND "+component_name+" model Camera01 Hist";
  beacon_calibrator_camera01_live_command_ = "COMMAND "+component_name+" model Camera01 Live";
  ...
  int64_t iteration_interval = 1000000 / int64_t(filter_->loop_frequency());
  VLOG(1) << "Loop frequency and iteration interval = "
      << filter_->loop_frequency() << " " << iteration_interval;
  ...
  while (!filter_->exit_loop()) {
    ...
    if (run_state == kRunningModel) {
      ...
      // run model
      int64_t mod_iteration_end_ts = iteration_end_ts + historical_time_offset_;
      ...
      if (iteration_end_ts - last_results_publish_ts_ > results_publish_interval_) {
        if (model_->ResultsChanged()) {
          ...
          VLOG(1) << "No change in model result. Not publishing.";
        }
      }
