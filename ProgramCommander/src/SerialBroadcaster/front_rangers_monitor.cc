...
const std::string FrontRangersMonitor::RANGER_TYPE = "SonarOrLidar";

/** Constructor */
FrontRangersMonitor::FrontRangersMonitor(std::string programs_setup_filename,
    std::string component_name) {
  programs_setup_config_filename_ = programs_setup_filename;
  component_name_ = component_name;  
  is_initiated_ = false;
  if (!Initiate()) LOG(ERROR) << "Could not initiate FrontRangersMonitor.";
}

/** Destructor - all members are self destructing */
FrontRangersMonitor::~FrontRangersMonitor() {
  VLOG(1) << "FrontRangersMonitor shutdown.";
...
bool FrontRangersMonitor::Initiate() {
  if (!InitiateComponent()) {
    LOG(ERROR) << "Could not initiate FrontRangersMonitor component objects";
    return false;
  }
...
bool FrontRangersMonitor::InitiateComponent() {
...
bool FrontRangersMonitor::InitiateSerialMonitor() {
  ...
  pub_message_subject_ = component_name_ + " ";
  LOG(INFO) << "Front ranger messages will be published on subject = '" << pub_message_subject_;
  pub_message_subject_length_ = pub_message_subject_.length();
  log_level_ = 0;
...
bool FrontRangersMonitor::HandleCommand(StringPtrType cmd) {
  // exit command
...
bool FrontRangersMonitor::HandleStatusQuery(StringPtrType status_query) {
...
bool FrontRangersMonitor::StartLooping() {
...
FrontRangersMonitor::StringPtrType FrontRangersMonitor::AssembleStatusString() {
...
bool FrontRangersMonitor::ReadDataAndPublish() {
  ...
  if (n < 0) {
    ...
    if (n==15 && buf[0]=='$') {
      ...
      uint8_t ranger_id = 0;
      uint32_t range = 0;
      uint32_t delay_since_trigger = 0;
      int8_t code = 0;
      packet_count = uint8_t(buf[1]);
      ranger_id = uint8_t(buf[2]);
      range = uint32_t((buf[3]<<24) | (buf[4]<<16) | (buf[5]<<8) | (buf[6]));
      delay_since_trigger = uint32_t((buf[7]<<24) | (buf[8]<<16) | (buf[9]<<8) | (buf[10]));
      code = int8_t(buf[11]);
      ...
      if (log_level_>0) std::cout << " ranger_id = " << int(ranger_id)
          << " range=" << range << " delay=" << delay_since_trigger
          << " code = " << int(code) << " count = " << int(packet_count);
          
      // Should we set the type of ranger - Sonar or Lidar here? Or should this be a configuration in the filter?
      
      // Assemble message
      range_message_.Clear();
      anantak::HeaderMsg* hdr_msg = range_message_.mutable_header();
      anantak::RangeMessage* range_msg = range_message_.mutable_range_msg();
      // Set header
      hdr_msg->set_timestamp(recieve_time);
      hdr_msg->set_type(RANGER_TYPE);
      hdr_msg->set_recieve_timestamp(recieve_time);
      hdr_msg->set_send_timestamp(recieve_time);
      // Set message
      range_msg->set_ranger_num(ranger_id);
      range_msg->set_range(range);
      range_msg->set_delay(delay_since_trigger);
      range_msg->set_code(code);
      
      // Send message
      SendSensorMessage(range_message_, pub_message_subject_, message_publisher_);
...
bool FrontRangersMonitor::SendSensorMessage(const anantak::SensorMsg& sensor_msg,
...
bool FrontRangersMonitor::ConnectToPort(const std::string& portname) {
...
bool FrontRangersMonitor::DisconnectFromPort() {
  if (is_connected) {
    close(USB);
