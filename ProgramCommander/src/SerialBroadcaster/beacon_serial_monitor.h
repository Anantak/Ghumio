/** Beacon Serial monitor component
 * Connects to the serial port for Beacon IMU. Gets the messages, broadcasts them.
 */
...
class BeaconSerialMonitor {
 public:
  /** Constructor - gets the setupfile name and the component name */
  BeaconSerialMonitor(std::string programs_setup_filename, std::string component_name);
  
  /** Destructor - generally everything must destruct on its own */
  virtual ~BeaconSerialMonitor();
 ...
 private:
  ...
  std::string imu_message_subject_;
  size_t imu_message_subject_length_;
  ...
  static const std::string IMU_TYPE;
  static const int32_t IMU_NUM;
  anantak::SensorMsg imu_message_;
};
