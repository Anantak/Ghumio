...
int main(int argc, char** argv) {
  ...
  anantak::ShowSaveImages show_images(programs_setup_filename, component_name);
  // Check if filter was created alright
  if (!show_images.is_initiated()) {
    LOG(ERROR) << "An error occurred, could not initiate sync images broadcaster. Exiting.";
    return -1;
  }
  // Start looping
  if (!show_images.StartProcessingImages()) {
