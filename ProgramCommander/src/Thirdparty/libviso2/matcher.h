...
public:
  ...
  // return reference to vector with matched feature points and indices
  // this saves a vector copy, but the caller should make sure not to call after matcher is deleted
  const std::vector<Matcher::p_match>& getMatchesByRef() { return p_matched_2; }
